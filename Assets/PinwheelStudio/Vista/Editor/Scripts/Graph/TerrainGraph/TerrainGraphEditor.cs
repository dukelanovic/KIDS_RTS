        ...
        internal override void ExecuteGraph()
        {
            ...
            if (ids.Count > 0)
            {
                ...
                if (m_externalInputProvider != null)
                {
                    ...
                    {
                        Debug.LogWarning($"Failed to set external input for the graph {sourceGraph.name}. Make sure the corresponding biome exists and active in the scene. Try closing and reopening the graph editor.");
                    }
                }
