...
{
	[CreateAssetMenu(fileName = "SoundLibrary", menuName = Storage.AssetName + "/Sound Library")]
	public class SoundLibrary : ScriptableObject
	{
		public float UnitMaxSoundDistance = 23f;
		public float UnitMinSoundDistance = 3f;
		...
		{
			var soundsList = new List<string> { "None or Missing" };
		...
		{
			var pathSplitted = path.Split(new [] { '/' }, System.StringSplitOptions.RemoveEmptyEntries);
		...
		public static AudioSource MakeUnitAudio(Unit unit, bool tryGetFirst = false)
		{
			var go = unit.gameObject;
			var audio = tryGetFirst ? go.CheckComponent<AudioSource>(true) : go.AddComponent<AudioSource>();
			
			audio.dopplerLevel = 0;
			audio.minDistance = GameController.Instance.MainStorage.soundLibrary.UnitMinSoundDistance;
			audio.maxDistance = GameController.Instance.MainStorage.soundLibrary.UnitMaxSoundDistance;
