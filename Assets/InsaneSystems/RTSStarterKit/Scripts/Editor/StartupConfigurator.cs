...
namespace InsaneSystems.RTSStarterKit
{
	...
	{
		static readonly bool isFirstRun;
		...
		static Startup()
		{
			...
			if (!PlayerPrefs.HasKey("EditorScenesInitialized"))
			{
				try
				{
					...
					if (settings.FindAll(sceneSettings => sceneSettings.path == sceneToAdd.path).Count == 0)
		...
		public static void CreateLayer(int atPosition, string name)
		{
			...
			var targetProp = layerProps.GetArrayElementAtIndex(atPosition);
